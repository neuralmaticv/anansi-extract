...
parser = argparse.ArgumentParser(
    description="Pot(j)era videos batch processor", formatter_class=argparse.ArgumentDefaultsHelpFormatter
)
...
for file in videoFiles:
    ...
    try:
        os.system(
            'python potera-single-video.py -srcdir "%s" -file "%s" -o "%s" -lang "%s" -csv "%s" -d %s'
            % (directoryWithVideoFiles, file, directoryOutput, language, csvFileName, createDebugData)
        )
