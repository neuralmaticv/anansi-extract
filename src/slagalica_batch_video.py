...
parser = argparse.ArgumentParser(
    description="Slagalica videos batch processor", formatter_class=argparse.ArgumentDefaultsHelpFormatter
)
...
for file in videoFiles:
    ...
    try:
        os.system(
            'python slagalica-single-video.py -srcdir "%s" -file "%s" -o "%s" -lang "%s" -csv "%s" -d %s -showt %s'
            % (directoryWithVideoFiles, file, directoryOutput, language, csvFileName, createDebugData, snowtime)
        )
