...
def areAllPointsInsideSeekBorderArea(
    approx, seekAreaBorderHorizontalY, seekAreaBorderHorizontalXStart, seekAreaBorderHorizontalXEnd
):
    result = True
    n = approx.ravel()
    i = 0
    ...
    for j in n:
        if i % 2 == 0:
            ...
            if writeDebugInfoOnImages:
                ...
                cv2.putText(original_img_preview, textCoordinate, (x, y), font, 0.5, (0, 0, 255))
        i = i + 1
...
def preprocessBeforeOCROld(imageToProcess, invertColors):
    ...
    imageToProcess = cv2.normalize(diff_img, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1)
...
if preprocessQuestionImageBeforeOCR:
    questionRectangleImage = preprocessBeforeOCR(
...
if preprocessAnswerImageBeforeOCR:
    answerRectangleImage = preprocessBeforeOCR(
