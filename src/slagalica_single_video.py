...
gameRecognitionIntroThreshold = (
    0.8  # 1.0 is =width*height (0.8 is good for new game intro, 0.1 is good for old game intro behind tv host)
)
...
thresholdInNumberOfPixelsDifferenceInAnswerRectanglePercentage = (
    0.007  # (1.0 is width * height of the answer rectangle)
)
...
parser = argparse.ArgumentParser(
    description="Slagalica single video processor", formatter_class=argparse.ArgumentDefaultsHelpFormatter
)
...
parser.add_argument(
    "-d",
    "--debugData",
    help="create frame image files for every image processed. note: can use up a lot of data space!",
    default="True",
)
parser.add_argument(
    "-showt", "--showtime", help="create windows and preview of everything that is happening", default="True"
)
parser.add_argument(
    "-poi",
    "--preprocessOCRImages",
    help="apply processing (blur, threshold, etc.) before doing ocr to images",
    default="True",
)
parser.add_argument(
    "-feocr", "--forceEasyOCR", help="force using of slower EasyOCR instead of default pytesseract", default="False"
)
...
def print_progress_bar(index, total, label, endlabel):
    ...
    sys.stdout.write("\r")
    sys.stdout.write(
        f"[{'=' * int(n_bar * progress):{n_bar}s}] {int(100 * progress)}%  {label} {index}/{total} {endlabel}"
    )
    sys.stdout.flush()
...
answerRectTemp = originalFrame[
    seekAreaQuestionBorderLowerLineY:seekAreaAnswerBorderLowerLineY, seekAreaBorderLeftX:seekAreaBorderRightX
].copy()
answer_imageHeight, answer_imageWidth, _ = answerRectTemp.shape
thresholdInNumberOfPixelsDifferenceInAnswerRectangleValue = int(
    thresholdInNumberOfPixelsDifferenceInAnswerRectanglePercentage * answer_imageHeight * answer_imageWidth
)
print(
    "Number of pixels for change detection in the answer rectangle: %d"
    % thresholdInNumberOfPixelsDifferenceInAnswerRectangleValue
)
...
if imageHeight == 1080:
    ...
    if thresholdConfidenceLevelTemplateMatchingDesiredGameIntro is None:
        thresholdConfidenceLevelTemplateMatchingDesiredGameIntro = (
            thresholdConfidenceLevelTemplateMatchingDesiredGameIntro1080p
        )
    ...
    if thresholdConfidenceLevelTemplateMatchingNextGameIntro is None:
        thresholdConfidenceLevelTemplateMatchingNextGameIntro = (
            thresholdConfidenceLevelTemplateMatchingNextGameIntro1080p
        )
    ...
    if thresholdConfidenceLevelTemplateMatchingDesiredGameIntro is None:
        thresholdConfidenceLevelTemplateMatchingDesiredGameIntro = (
            thresholdConfidenceLevelTemplateMatchingDesiredGameIntro720p
        )
    if templateToFindNextGameIntroImagePath is None:
        templateToFindNextGameIntroImagePath = templateToFindNextGameIntro720pImagePath
    if thresholdConfidenceLevelTemplateMatchingNextGameIntro is None:
        thresholdConfidenceLevelTemplateMatchingNextGameIntro = (
            thresholdConfidenceLevelTemplateMatchingNextGameIntro720p
        )
...
while success:
    ...
    if gameRecognitionLogicForceTemplateMode:
        gameStartFoundInCurrentFrame = match_image_template(
            originalFrame,
            templateToFindGameIntro,
            confidenceLevel=thresholdConfidenceLevelTemplateMatchingDesiredGameIntro,
        )
    else:
        gameStartFoundInCurrentFrame = isDesiredMaskFrameVisible(
            gameRecognitionIntroThreshold,
            game_intro_pink_mask_l_h,
            game_intro_pink_mask_l_s,
            game_intro_pink_mask_l_v,
            game_intro_pink_mask_u_h,
            game_intro_pink_mask_u_s,
            game_intro_pink_mask_u_v,
            originalFrame.copy(),
        )
    ...
    if gameFound:  # commonly known as "else"
        ...
        questionRectangleImage = originalFrame[
            seekAreaQuestionBorderUpperLineY:seekAreaQuestionBorderLowerLineY, seekAreaBorderLeftX:seekAreaBorderRightX
        ].copy()
        if showtimeMode:
            cv2.imshow("Question rectangle", questionRectangleImage)
            cv2.waitKey(1)
        answerRectangleImage = originalFrame[
            seekAreaQuestionBorderLowerLineY:seekAreaAnswerBorderLowerLineY, seekAreaBorderLeftX:seekAreaBorderRightX
        ].copy()

        questionFrameVisible = isDesiredMaskFrameVisible(
            percentageOfAreaThreshold,
            question_mask_blue_l_h,
            question_mask_blue_l_s,
            question_mask_blue_l_v,
            question_mask_blue_u_h,
            question_mask_blue_u_s,
            question_mask_blue_u_v,
            questionRectangleImage,
        )

        answerCurrentPreProccessed = preprocessGetReadyForOCR(
            answerRectangleImage.copy(),
            lower_bound=241,
            upper_bound=255,
            type=cv2.THRESH_BINARY,
            useGaussianBlurBefore=True,
            useBlurAfter=True,
        ).copy()
        ...
        if questionFrameVisible:
            if answerTemp is not None:
                ...
                diffSimilarityValueNumberOfPixels = compare_two_images_number_of_pixels(
                    answerTemp, answerCurrentPreProccessed
                )
        ...
        if questionWithAnswerFrameFound:
            if createDebugData:
                cv2.imwrite(
                    "%s/%s-q%d-%d-0-frame-original.jpg"
                    % (directoryOutput, fileName, numberOfFoundQuestionAnswerPair + 1, frameIndex),
                    originalFrame,
                )
                debugCopy = originalFrame.copy()
                cv2.line(
                    debugCopy,
                    (seekAreaBorderLeftX, seekAreaQuestionBorderUpperLineY),
                    (seekAreaBorderRightX, seekAreaQuestionBorderUpperLineY),
                    (0, 255, 0),
                    thickness=1,
                )
                cv2.line(
                    debugCopy,
                    (seekAreaBorderLeftX, seekAreaQuestionBorderLowerLineY),
                    (seekAreaBorderRightX, seekAreaQuestionBorderLowerLineY),
                    (0, 255, 255),
                    thickness=2,
                )
                cv2.line(
                    debugCopy,
                    (seekAreaBorderLeftX, seekAreaAnswerBorderLowerLineY),
                    (seekAreaBorderRightX, seekAreaAnswerBorderLowerLineY),
                    (0, 255, 0),
                    thickness=1,
                )
                cv2.line(
                    debugCopy,
                    (seekAreaBorderLeftX, seekAreaQuestionBorderUpperLineY),
                    (seekAreaBorderLeftX, seekAreaBorderLeftY),
                    (0, 255, 0),
                    thickness=1,
                )
                cv2.line(
                    debugCopy,
                    (seekAreaBorderRightX, seekAreaQuestionBorderUpperLineY),
                    (seekAreaBorderRightX, seekAreaBorderRightY),
                    (0, 255, 0),
                    thickness=1,
                )
                debugFrameName = "%s/%s-q%d-%d-1-frame-contours.jpg" % (
                    directoryOutput,
                    fileName,
                ...
                debugFrameName = "%s/%s-q%d-%d-2.1-question.jpg" % (
                    directoryOutput,
                    fileName,
                    numberOfFoundQuestionAnswerPair + 1,
                    frameIndex,
                ...
                debugFrameName = "%s/%s-q%d-%d-3.1-answer.jpg" % (
                    directoryOutput,
                    fileName,
                    numberOfFoundQuestionAnswerPair + 1,
                    frameIndex,
            ...
            if preprocessImageBeforeOCR:
                questionRectangleImage = preprocessGetReadyForOCR(
                    questionRectangleImage.copy(),
                    lower_bound=questionLowerBoundGlobalTreshold,
                    upper_bound=questionUpperBoundGlobalTreshold,
                    type=cv2.THRESH_BINARY,
                    useGaussianBlurBefore=True,
                    useBlurAfter=True,
                )

                answerRectangleImage = preprocessGetReadyForOCR(
                    answerRectangleImage.copy(),
                    lower_bound=answerLowerBoundGlobalTreshold,
                    upper_bound=answerUpperBoundGlobalTreshold,
                    type=cv2.THRESH_BINARY,
                    useGaussianBlurBefore=True,
                    useBlurAfter=True,
                )
            ...
            debugFrameName = "%s/%s-q%d-%d-2.2-question.jpg" % (
                directoryOutput,
                fileName,
                numberOfFoundQuestionAnswerPair + 1,
                frameIndex,
            ...
            debugFrameName = "%s/%s-q%d-%d-3.2-answer.jpg" % (
                directoryOutput,
                fileName,
                numberOfFoundQuestionAnswerPair + 1,
                frameIndex,
            ...
            if howManyFramesToJumpAfterSuccess > 0:
                ...
                print(
                    "\nJumping to %dth frame of %d after found question/answer..."
                    % (frameIndex, videoFileFramesTotalLength)
                )
        ...
        if gameRecognitionLogicForceTemplateMode:
            gameOutroFoundInCurrentFrame = match_image_template(
                originalFrame,
                templateToFindNextGameIntro,
                confidenceLevel=thresholdConfidenceLevelTemplateMatchingNextGameIntro,
            )
        else:
            gameOutroFoundInCurrentFrame = isDesiredMaskFrameVisible(
                gameRecognitionOutroThreshold,
                game_outro_blue_mask_l_h,
                game_outro_blue_mask_l_s,
                game_outro_blue_mask_l_v,
                game_outro_blue_mask_u_h,
                game_outro_blue_mask_u_s,
                game_outro_blue_mask_u_v,
                originalFrame.copy(),
            )
...
with open(csvLogFileLocation, "a+", encoding="UTF8", newline="") as f:
    ...
    csvDataRow = [
        filePath,
        numberOfFoundQuestionAnswerPair,
        durationTextFormat,
        videoAverageFps,
        bitrate,
        imageWidth,
        imageHeight,
        howManyFramesToIterateBy,
        duration,
    ]
    writer.writerow(csvDataRow)
