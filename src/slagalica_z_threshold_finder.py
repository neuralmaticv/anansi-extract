...
while True:
    ...
    image1Processed = preprocessBeforeOCR(
        image.copy(), global_treshold_lower, global_treshold_upper, cv2.THRESH_BINARY, gaussan_blur_on, median_blur_on
    )
    cv2.imshow("Image1 global threshold", image1Processed)

    image1Processed2 = preprocessBeforeOCR(
        image.copy(),
        global_treshold_lower,
        global_treshold_upper,
